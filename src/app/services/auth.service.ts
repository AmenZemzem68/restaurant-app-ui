...
export class AuthService {
  ...
  private baseUrl: string = `${this.host}:5176/api/Client`;
  ...
  login(loginObj: any, isFacebookLogin: boolean): Observable<any> {
    return this.http
      .post<any>(
        `${this.host}:8070/api/Auth/authenticate?isFacebookLogin=${isFacebookLogin}`,
        loginObj
      )
      .pipe(take(1));
  }
  ...
  register(formData: FormData): Observable<any> {
    return this.http
      .post<any>(`${this.baseUrl}/register`, formData)
      .pipe(take(1));
  }
