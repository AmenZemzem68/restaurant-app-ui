...
export class CommandeService {
  ...
  private baseUrl: string = `${this.host}:5176/api`;
  ...
  updateCommande(commande: any): Observable<any> {
    return this.http.put<any>(
      `${this.baseUrl}/Commande/${commande.id}`,
      commande
    );
  }
  ...
  addLigneCommande(lineData: any): void {
    this.http
      .post(`${this.baseUrl}/LigneCommande`, lineData)
      .subscribe((response) => {
  ...
  updateLigneCommande(ligneCommande: any): Observable<any> {
    return this.http.put<any>(
      `${this.baseUrl}/LigneCommande/${ligneCommande.id}`,
      ligneCommande
    );
  }
  ...
  updateCommandeState(id: number, newState: string): Observable<any> {
    ...
    return this.http.put<any>(
      `${this.baseUrl}/Commande/updateState/${updatedCommande.id}`,
      updatedCommande
    );
  }
}
